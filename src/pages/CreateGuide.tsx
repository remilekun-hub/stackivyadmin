...
import { GuideCreateType, base_url } from "../../types";
import {
  Select,
  SelectContent,
  SelectGroup,
...
function CreateGuide() {
  ...
  const [guideData, setGuideData] = useState<GuideCreateType>({
    title: "",
    summary: "",
    description: "",
    categories: [],
  ...
  const handlePublishGuide = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (guideData.saved === "true") {
      await SaveToDraft();
    } else {
      await publishPost();
    }
  ...
  const publishPost = async () => {
    if (
      ...
      toast.error("All fields must be filled", { id: "guide" });
      return;
    }
    try {
      toast.loading("creating guide..", { id: "guide" });
      const form_Data = new FormData();
      if (file) {
      ...
      form_Data.append("saved", guideData.saved);
      form_Data.append("categories", JSON.stringify(guideData.categories));
      form_Data.append("guide_contents", JSON.stringify({ post: value }));
      ...
      if (data.code === 200) {
        toast.success("guide created Successfully", { id: "guide" });
        setGuideData({
          title: "",
          description: "",
          summary: "",
      ...
      if (data.code != 200) {
        toast.error("couldn't create guide at this time", { id: "guide" });
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const SaveToDraft = async () => {
    if (
      ...
      toast.error("All fields must be filled", { id: "guideDraft" });
      return;
    }
    try {
      toast.loading("saving to draft..", { id: "guideDraft" });
      const form_Data = new FormData();
      if (file) {
        form_Data.append("image", file);
      }
      ...
      if (!data) {
        toast.error("something went wrong", { id: "guideDraft" });
      }
      if (data.code === 200) {
        toast.success("Draft saved successfully", { id: "guideDraft" });
        setGuideData({
          title: "",
          description: "",
          summary: "",
      ...
      if (data.code != 200) {
        toast.error("couldn't save draft at this time", {
          id: "guideDraft",
        });
      }
    } catch (error) {
        ...
        >
          ...
          <ScrollArea className=" xl:basis-[350px] h-full bg-white rounded-[16px] border-[1px] border-[#F3F4F6] px-4 lg:px-5 pt-5">
            <div className="flex gap-7 mb-6">
              ...
              <button
                onClick={() => setGuideData({ ...guideData, saved: "true" })}
              >
                Save to draft
              </button>
            </div>
            ...
            <div>
              ...
              <h1 className="mb-3">Guide Summary</h1>
              <textarea
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked === true) {
                          ...
                          const filteredData = guideData.categories.filter(
                            (cat) => cat != c
                          );
                          setGuideData({
                            ...guideData,
